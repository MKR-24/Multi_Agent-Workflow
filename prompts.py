...
def plan_prompt(state: State) -> HumanMessage:
    ...
    enabled_for_planner = [
        ...
        if a in ("web_researcher","chart_generator", "synthesizer")
    ]
...
def executor_prompt(state: State) -> HumanMessage:
    ...
    executor_prompt = f"""
        ...
        `{ '`, `'.join(sorted(set([a for a in _get_enabled_agents(state) if a in ['web_researcher','chart_generator','chart_summarizer','synthesizer']] + ['planner']))) }`.
        ...
        **Guidelines**
        ...
        {{
        "replan": <true|false>,
        "goto": "<{ '|'.join([a for a in _get_enabled_agents(state) if a in ['web_researcher','chart_generator','chart_summarizer','synthesizer']] + ['planner']) }>",
        "reason": "<1 sentence>",
        "query": "<text>"
        }}
        ...
        **PRIORITIZE FORWARD PROGRESS:** Only replan if the current step is completely blocked.
        ...
        is `web_researcher`, the query should be a standalone question, 
